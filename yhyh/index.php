...
if(eregi("/yhyh/index.php", $PHP_SELF)) {
	if(!$_yhyh_common_load) {
		include_once("./module/inc/root_info.php");
		include_once("./module/inc/common.php");
	}
	$_solo_mode = true;
}
...
?><!doctype html><html><head>
	...
	<meta charset="utf-8"><title><?=$_board_title?></title><?
	...
	if($_grant) {
		...
		if(!$s_grant && (($_config->yhb_use_list == 1 && $_REQUEST["_no"]) || !$_REQUEST["_no"])) {
			...
			while($bb = $_LhDb->Fetch_Object($result)) {
				$_modify_link = $_modify_link_original = ($_rewrite_mod) ? "/yh/write/modify/".$_REQUEST["_id"]."/".$bb->yhb_number : $PHP_SELF."?".$query_string."&_module=write&_writeMode=modify&_no=".$bb->yhb_number;
				$_delete_link = "javascript:Rows_Delete_Check('".$_REQUEST["_no"]."');";
				$_delete_link_original = ($_rewrite_mod) ? "/yh/delete/".$_REQUEST["_id"]."/".$bb->yhb_number : $PHP_SELF."?".$query_string."&_module=delete_proc&_no=".$bb->yhb_number;
