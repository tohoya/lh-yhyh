...
if($_REQUEST["_id"]) {
	...
	$where .= " AND yhb_number = board_no AND s_name != '' AND (f_ext == 'png' && f_ext == 'jpg' && f_ext == 'bmp' && f_ext == 'jpeg' && f_ext == 'gif' && f_ext == 'pic') GROUP BY yf.board_no ORDER BY yf.f_order ASC";
	...
	$result_total = $_LhDb->Query($query_gallery);
	$_total_rows = 0;
	
	while($bb = $_LhDb->Fetch_Object($result_total)) {
		$f_query = "select s_name, f_ext from yh_file where board_no = '".$bb->yhb_number."' order by f_order";
		if($_LhDb->Query_Row_Num($f_query) > 0) {
			...
			while($f_data = $_LhDb->Fetch_Object($f_result)) {
				switch(strtolower($f_data->f_ext)) {
					...
					case "gif":
					case "pic":
						if(file_exists(_lh_document_root._lh_yhyh_web."/upload/".$f_data->s_name)) {
							$file_check = true;
						}
					break;
				}
	...
	$query_gallery .= " order by yhb_notice desc, yhb_number_up desc, yhb_number_rows asc limit ".$_REQUEST["_start_idx"].", ".$_REQUEST["_limit"];
	
	//echo $query;
	$result_gallery = $_LhDb->Query($query_gallery);
		...
		if($_LhDb->Query_Row_Num($f_query) > 0) {
			...
			while($f_data = $_LhDb->Fetch_Object($f_result)) {
				switch(strtolower($f_data->f_ext)) {
					...
					case "pic":
						if(file_exists(_lh_document_root._lh_yhyh_web."/upload/".$f_data->s_name)) {
							$result->file[$f_i]->f_name = $f_data->f_name;
							$result->file[$f_i]->s_name = $f_data->s_name;
							$result->file[$f_i]->url = _lh_yhyh_web."/upload/".$f_data->s_name;
							$result->file[$f_i]->f_size = $f_data->f_size;
							$result->file[$f_i]->f_ext = $f_data->f_ext;
							$result->file[$f_i]->f_download = $f_data->f_download;
							$result->file[$f_i]->f_order = $f_data->f_order;
							$f_i++;
						}
					break;
				}
