	...
	<div class="yhyh_view_body">
		...
		<div class="yhyh_ari">
			...
			<div class="yhyh_info_right">
				<? if($_result->ip) { ?><label>IP : </label>&nbsp;<span>(<?=$_LhDb->Get_Split($_result->ip, 0, "\.").".".$_LhDb->Get_Split($_result->ip, 1, "\.").".".$_LhDb->Get_Split($_result->ip, 2, "\.").".".$_LhDb->Text_Replace($_LhDb->Get_Split($_result->ip, 3, "\."))?>)</span>, <? } ?>
		...
		<div class="yhyh_social">
			<span class="fb-like" data-href="<?=urldecode($_protocol)?><?=urldecode($_SERVER['HTTP_HOST'])?><?=urldecode($_SERVER['REQUEST_URI'])?>" data-send="false" data-layout="button_count" data-width="450" data-show-faces="true"></span>
		</div>
		<div class="yhyh_view_content">
			<iframe id="content_frame" src="<?=_lh_yhyh_web?>/?_module=view&_img_width=auto&_content_frame=content_frame&_no=<?=$_result->number?>" frameborder="0"></iframe>
		</div>
		<?
		/*
