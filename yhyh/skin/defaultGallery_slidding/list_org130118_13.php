...
var Choice_Photo_View = function(no, scroll_type) {
	...
	var Image_Loader = function() {
		$(".yhyh_thum_title > a").attr("href", oThum.data("yhb_title_link").toString().replace("amp;", "")).html(oThum.data("yhb_title"));
...
var Page_Gallery_Loader = function(start_idx, type, choice_idx) {
	...
	$.post("<?=_lh_yhyh_web?>/", { _module : "gallery_list_proc", _id : "<?=$_REQUEST["_id"]?>", _width : _width, _height : _height, _start_idx : start_idx, _limit : slide_show_count }, function(data) {
		...
		$("> ul", vDiv).each(function(i) {
			...
			var oThum = $("<div/>")
			.data({
				"yhb_no" : $("> ._yhb_no", this).html()
				, "yhb_number" : $("> ._yhb_number", this).html()
				, "yhb_title" : $("> ._yhb_title", this).html()
				, "yhb_title_link" : "<?=$PHP_SELF?>?<?=$query_string?>&_no=" + $("> ._yhb_number", this).html()
				, "yhb_name" : $("> ._yhb_name", this).html()
				, "yhb_date" : $("> ._yhb_date", this).html()
				, "yhb_hit" : $("> ._yhb_hit", this).html()
			...
			if($.browser.msie) {
				...
				if(oThum_img.width() == 0) oThum_img.width(Number(oThum.data("f_width")));
				if(oThum_img.height() == 0) oThum_img.height(Number(oThum.data("f_height")));
			...
			.animate({
				...
				$("._load_count").html(thum_container.children().length);
...
<div class="yhyh_list">
	...
	<div class="yhyh_list_header">
		<p class="numberFont">Gallery : <span class="_load_count">0</span>/<?=number_format($_total_rows)?></p>
