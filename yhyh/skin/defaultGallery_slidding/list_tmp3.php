...
var Page_Change_Action = function(type) {
	var load_count = $("> .thum_container > div", ".yhyh_thum_slider").length, eq = 0, choice_idx = 1, start_idx, slide_eq, oThum, slide_left = 0;
	var page = Math.ceil(load_count / slide_show_count);
	switch(type) {
		case "next":
			...
			choice_idx = (eq < load_count) ? eq + 1 : load_count;
		break;
		case "prev":
			page = Math.ceil(choice_no / slide_show_count);
			choice_idx = (page - 1) * slide_show_count;
			slide_eq = (page - 2) * slide_show_count;
			oThum = $("> .thum_container > div:eq(" + slide_eq + ")", ".yhyh_thum_slider");
			slide_left = oThum.position().left;
		break;
	}
	...
	$(".yhyh_thum_slider").stop().animate({
		...
		if(type == "next") {
			if(eq < load_count) {
				Choice_Photo_View(choice_idx);
			} else {
				Page_Gallery_Loader(start_idx);
			}
		} else {
			Choice_Photo_View(choice_idx);
		}
