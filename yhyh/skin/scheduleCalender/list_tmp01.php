...
function Schedule_Init(year, month) {
	...
	var c_type = (c_date.getMilliseconds() > (new Date(_year, _month)).getMilliseconds()) ? 1 : -1;
	...
	if(_container.children().length > 2) {
		$("> div", _container).each(function(i) {
			...
			$(this).stop().animate({
				top : move_top
				, opacity : (i == _container.children().length - 1) ? 1 : disabled_opacity
	...
	if(c_type < 0) {
		...
		if(!_div_n) {
			_div_n = $("<div/>").stop().fadeTo(0, disabled_opacity).appendTo(_container).css({"z-index" : "1"});
			for(var d = 1; d <= total_day.prev; d++) {
				Day_Init(year_n, month_n, d, _div_n);
			}
			var div_n_top = Number(_div_n.data("week_count")) * _day_height * -1;
			if(week_first == 0) {
				div_n_top -= _day_height;
			}
			_div_n.css({"top": (div_n_top) + "px"});
		} else {
		...
		if(!_div_t) {
			...
			div_p_top = Number(_div_t.data("week_count")) * _day_height;
		} else {
			_div_t.css({"z-index" : "2"});
		}
		...
		_div_p = $("<div/>").stop().fadeTo(0, disabled_opacity).appendTo(_container).css({"top": div_p_top + "px", "z-index" : "1"});
		
		for(var d = 1; d <= total_day.next; d++) {
			Day_Init(year_p, month_p, d, _div_p);
		}
		
		var div_p_move = (Number(_div_t.data("week_count")) * _day_height);
		if(week_last == 6) {
