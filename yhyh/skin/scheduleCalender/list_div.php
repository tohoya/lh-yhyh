...
function View_Pop_Link(div) {
	...
	, { duration : 200, ease : "easeOutCubic", type : "show", completeFunction : function() {
...
function Write_Pop_Action(url) {
	...
	Schedule_Pop_Show("show"
	, url
	, { duration : 200, ease : "easeOutCubic", type : "show", completeFunction : Write_Pop_Load_Complete }
	);
}
...
function Schedule_Get_List(y, m, d) {
	...
	$.post(url, params, function(data) {
		...
		$("> row", list).each(function(i) {
			...
			var _oA = $("<a/>").attr("href", "javascript:;").appendTo(_Li);
			...
			$("<span/>").html(s_date.getFullYear() + "." + s_moon + "." + s_day).appendTo(_oA);
			$("<span/>").html("~").appendTo(_oA);
			$("<span/>").html(d_date.getFullYear() + "." + d_moon + "." + d_day).appendTo(_oA);
			$("<span/>").html($(this).attr("yhb_title")).appendTo(_oA);
		});
	});
}
