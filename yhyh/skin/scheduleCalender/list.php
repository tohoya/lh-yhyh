...
$(window).load(function() {
	...
	Schedule_Resize();
	var sc = $("#schedule_layout");
	if($.browser.mozilla) {
		sc.mousewheel(function(objEvent, intDelta) {
			Schedule_Move(intDelta > 0 ? "prev" : "next");
		})
		.hover(function(){
			$(window).mousewheel(function() { return false; });
		}, function(){
			$(window).unbind("mousewheel");
		});
	} else {
		sc.hover(function(){
			$("body").get(0).onmousewheel = function() { return false; };
		}, function(){
			$("body").get(0).onmousewheel = null;
		})
		.get(0).onmousewheel = function(e) {
...
function Schedule_Init(year, month) {
	...
	if(_div_t.length == 0) {
		...
		//Schedule_Load_Get_List(year, month, "", "", "", "");
	}
	...
	if(_div_p.length == 0) {
		...
		//Schedule_Load_Get_List(year_p, month_p, "", "", "", "");
	}
	...
	if(_div_n.length == 0) {
		...
		//Schedule_Load_Get_List(year_n, month_n, "", "", "", "");
	}
	_container.data({
		year_p : year_p
		, month_p : month_p
	...
	$("> div", _container).each(function(i) {
		...
		$(this).stop().animate({
			...
			//alert(month_p + " / " + month_n);
//			Schedule_Load_Get_List(year_p, month_p, 1, year_n, month_n, total_day.next);
...
function Schedule_Load_Get_List() {
	var _schedule = $("#schedule_layout");
	var _container = $(".day_container", _schedule);
	
	y = Number(_container.data("year_p"));
	m = Number(_container.data("month_p"));
	d = Number(_container.data("date_p"));
	ey = Number(_container.data("year_n"));
	em = Number(_container.data("month_n"));
	ed = Number(_container.data("date_n"));
	...
	$.post(url, params, function(data) {
		var list = $("> list", $.parseXML(data)), _schedule_day, _Ul;
		
		if($("> row", list).length == 0) {
			var s_date = Math.floor((new Date(y, m - 1, d, 0, 0, 0)).getTime() * 0.001);
			var e_date = (ey && em && ed) ? Math.floor((new Date(ey, em - 1, ed, 23, 59, 59)).getTime() * 0.001) : Math.floor((new Date(y, m - 1, d, 23, 59, 59)).getTime() * 0.001);
			if(s_date <= e_date) {
				for(var i = s_date; i <= e_date; i+=86400) {
					var show_date = new Date(i * 1000);
					_schedule_day = $("#calendar_day_" + show_date.getFullYear() + "_" + (show_date.getMonth() + 1) + "_" + show_date.getDate() + " > a");
					if(_schedule_day.length > 0) {
						_schedule_day.empty();
					}
				}
			}
		...
		$("> row", list).each(function(i) {
			var s_date = Number($(this).attr("reset_start"));
			var e_date = Number($(this).attr("reset_end"));
			var s_time = new Date(s_date * 1000);
			var e_time = new Date(e_date * 1000);
			if(s_date <= e_date) {
				for(var i = s_date; i <= e_date; i+=86400) {
					var show_date = new Date(i * 1000);
					_schedule_day = $("#calendar_day_" + show_date.getFullYear() + "_" + (show_date.getMonth() + 1) + "_" + show_date.getDate() + " > a");
		...
		$("> row", list).each(function(i) {
			var s_date = Number($(this).attr("yhb_start_time"));
			var e_date = Number($(this).attr("yhb_end_time"));
			if(s_date <= e_date) {
				for(var i = s_date; i <= e_date; i+=86400) {
					var show_date = new Date(i * 1000);
					_schedule_day = $("#calendar_day_" + show_date.getFullYear() + "_" + (show_date.getMonth() + 1) + "_" + show_date.getDate() + " > a");
					if($("> ul", _schedule_day).length == 0) {
						_Ul = $("<ul/>").appendTo(_schedule_day);
					} else {
						_Ul = $("> ul", _schedule_day);
					}
					
					var _Li = $("<li/>").css("color", $(this).attr("yhb_category_color")).appendTo(_Ul);
					$("<span/>").html("&bull;&nbsp;").appendTo(_Li);
					$("<span/>").html($(this).attr("yhb_title")).appendTo(_Li);
				}
			}
		});
	});
...
function Schedule_Get_List(y, m, d) {
	...
	$.post(url, params, function(data) {
		...
		$("> row", list).each(function(i) {
			...
			_Li_title
			.click(function(e) {
				if(e.srcElement.tagName.toString().toLowerCase() == "a") return;
				
				$("> li", _schedule_list).each(function(i) {
					var index = _Li_content.parents("li").index();
					var _content = $(" > ul > .view_pop_list_content", this);
					var _title = $(" > ul > .view_pop_list_title", this);
					if(index == i) {
						if(_title.data("menu_select") != "true") {
							_title.data("menu_select", "true").addClass("a_button");
						} else {
							_title.data("menu_select", "false").removeClass("a_button");
						}
			...
			.hover(function() {
				...
				if($(this).data("menu_select") != "true") {
					$(this).removeClass("a_button");
				}
			});

			
			$("<span/>").css("color", $(this).attr("yhb_category_color")).html("&bull;").appendTo(_Li_title);
			$("<span/>")
			...
			.click(function() {
				if(Rows_Delete_Check) {
					Rows_Delete_Check(yhb_number, "이 일정을 삭제하시겠습니까?");
				}
			})
